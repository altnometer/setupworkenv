...
(defvar xah--dvorak-to-mybeakl10-kmap
  '(("." . "o")
    ("," . "h")
    ("'" . "q")
    ("a" . "y")
    ...
    ("y" . "x")
    ("z" . "k"))
  "A alist, each element is of the form(\"e\" . \"d\"). First char is Dvorak, second is corresponding BEAKL10. Not all chars are in the list. When not in this alist, they are assumed to be the same.")
...
Version 2017-01-21"
  ...
  (xah-fly--define-keys
   ...
   '(
     ...
     ("'" . xah-reformat-lines)         ; q
     ("," . xah-shrink-whitespaces)     ; h
     ("-" . xah-cycle-hyphen-underscore-space)
     ("." . xah-backward-kill-word)     ; o
     (";" . xah-comment-dwim)
     ("/" . hippie-expand)
     ("\\" . nil)
     ...
     ("a" . execute-extended-command)                 ; y
     ("b" . isearch-forward)                          ; b
     ("c" . previous-line)                            ; n
     ("d" . xah-beginning-of-line-or-block)           ; c
     ("e" . xah-delete-backward-char-or-bracket-text) ; e
     ("f" . undo)                                     ; g
     ("g" . backward-word)                            ; d
     ("h" . backward-char)                            ; s
     ...
     ("l" . xah-insert-space-before)      ; v
     ("m" . xah-backward-left-bracket)    ; p
     ("n" . forward-char)                 ; t
     ("o" . open-line)                    ; i
     ("p" . xah-kill-word)                ; u
     ("q" . xah-cut-line-or-region)       ; /
     ("r" . forward-word)                 ; m
     ("s" . xah-end-of-line-or-block)     ; w
     ("t" . next-line)                    ; r
     ("u" . xah-fly-insert-mode-activate) ; a
     ("v" . xah-forward-right-bracket)    ; f
     ("w" . xah-next-window-or-frame)     ; l
     ("x" . xah-toggle-letter-case)       ; z
     ("y" . set-mark-command)             ; x
     ("z" . xah-goto-matching-bracket)))  ; k
